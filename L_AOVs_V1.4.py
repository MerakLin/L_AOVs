...
def SelectLayers(node):
    ...
    all_layers_count=0
    p.addBooleanCheckBox('Postage_Stamp',False)
    ...
    if selectlayer_sult==0:
        Return
         ...
         for a in range(len(selectlayer_sult)):
            ...
            shuffle_temp=dot
            unpremult=nuke.nodes.Unpremult()
            shuffle_xpos=shuffle.xpos()
            shuffle_ypos=shuffle.ypos()
            if postage_stamp_sult==0:
             unpremult.setXYpos(shuffle_xpos,shuffle_ypos+50)
            else:
             unpremult.setXYpos(shuffle_xpos,shuffle_ypos+100)
            unpremult.setInput(0,shuffle)   
            b=b+1
